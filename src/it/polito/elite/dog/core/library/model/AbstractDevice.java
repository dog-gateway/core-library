 ...
 * 
 */
public abstract class AbstractDevice extends ControllableDevice
        implements ServiceTrackerCustomizer<Object, Object>, Monitorable
{
    ...
    public void setDriver(Object obj)
    {
        ...
        properties.put("service.pid",
                AbstractDevice.toMonitorableId(this.deviceId));
        // Register a Monitorable service (MonitorAdmin bundle)
        this.serviceRegMonitorable = context
                .registerService(Monitorable.class.getName(), this, properties);
    ...
    /**
     * Sets a device online, i.e. connected to Dog and reachable or not. Does
     * not make sense if the device is not active.
     * 
     * @param online
     *            True if the device is connected to Dog and reachable, false
     *            otherwise.
     */
    public void setOnline(boolean online)
    {
        if (this.deviceProp.containsKey(DeviceCostants.ACTIVE) && Boolean
                .valueOf(this.deviceProp.get(DeviceCostants.ACTIVE).toString()))
        {
            // set the online property
            this.deviceProp.put(DeviceCostants.STATUS,
                    online ? DeviceCostants.STATUS_ONLINE
                            : DeviceCostants.STATUS_OFFLINE);
            // update the property
            this.serviceRegDevice.setProperties(this.deviceProp);
        }
    }
    ...
    /**
     * Check if this device is online, i.e., active, connected to Dog and
     * reachable, or not.
     * 
    ...
    public boolean isOnline()
    {
        ...
        Object onlineProp = this.deviceProp.get(DeviceCostants.STATUS);
        if (onlineProp != null
                && onlineProp.equals(DeviceCostants.STATUS_ONLINE))
        {
            online = true;
        }
        return online;
    ...
    protected void registerDevice(String deviceClass)
    {
        ...
        try
        {
            ...
            this.logger.log(LogService.LOG_INFO,
                    "REGISTERED DEVICE " + this.serviceRegDevice);
    ...
    public void unSetDriver(Driver driverRemoved)
    {
        ...
        {
            this.logger.log(LogService.LOG_INFO,
                    "DEACTIVATED DRIVER for " + this.deviceId);
    ...
    {
        this.logger.log(LogService.LOG_INFO,
                "UNREGISTERING device " + this.deviceId);
        ...
        catch (IllegalStateException ise)
        {
            this.logger.log(LogService.LOG_WARNING,
                    "The device was already unregistered");
        }
    }
    ...
    public void notifyEventAdmin(Notification notification)
    {
        ...
        Event notificationEvent = EventFactory.createNotificationEvent(
                notification, this.getClass().getSimpleName());

        // send
        this.postToEventAdmin(notificationEvent);
    }
            ...
            OffsetDateTime timestamp)
    {
        // Create the event through the DogEventFactory
        Event notificationEvent = EventFactory.createNotificationEvent(
                notification, timestamp, this.getClass().getSimpleName());
            ...
            OffsetDateTime timestamp, EventUrgency urgency)
    {
        // Create the event through the DogEventFactory
        Event notificationEvent = EventFactory.createNotificationEvent(
                notification, timestamp, urgency,
    ...
    /**
     * Method for sending a {@link Diagnostics} event to the OSGi Event Admin.
     * 
     * @param diagnostics
     *            The {@link Diagnostics} event to deliver.
     */
    public void notifyEventAdmin(Diagnostics diagnostics)
    {
        // Create the event through the DogEventFactory
        Event notificationEvent = EventFactory.createDiagnosticsEvent(
                diagnostics, this.getClass().getSimpleName());
     ...
     *            The {@link Diagnostics} event to deliver.
     * @param timestamp
     *            The timestamp associated to the {@link Diagnostics} event,
     *            overrides the {@link Diagnostics} creation time.
     */
    public void notifyEventAdmin(Diagnostics diagnostics,
            OffsetDateTime timestamp)
    {
        // Create the event through the DogEventFactory
        Event notificationEvent = EventFactory.createDiagnosticsEvent(
                diagnostics, timestamp, this.getClass().getSimpleName());
     ...
     *            overrides the {@link Diagnostics} creation time.
     * @param urgency
     *            The urgency of the diagnostics event, as an
     *            {@link EventUrgency} instance.
     */
    public void notifyEventAdmin(Diagnostics diagnostics,
            OffsetDateTime timestamp, EventUrgency urgency)
    {
     ...
     */
    public void notifyMonitorAdmin(DeviceStatus deviceStatus)
    {
    ...
    {
        this.logger.log(LogService.LOG_INFO,
                this.deviceId + " any drivers found!");
            ...
            throws IllegalArgumentException
    {
        ...
        if (id.equals("status"))
        {
            ...
            {
                logger.log(LogService.LOG_ERROR,
                        this.deviceId + " serialization error!");
            }
    ...
    private void postToEventAdmin(Event event)
    {
